...
const buildRow = (rec, rowId) => {
  ...
  const values = [
    ...
    sqlStr(rec.region),
    sqlStr(rec.deity),
    sqlStr(rec.tradition),
    sqlStr(rec.story),
    'NULL',                         // story_hi
    sqlStr(rec.highlight),
    'NULL',                         // highlight_hi
    sqlStr(rec.image),
    sqlStr(rec.credit),             // image_credit
    sqlStr(rec.creditUrl),          // image_credit_url
    sqlStr(rec.bestTime),           // best_time
    sqlStr(rec.timings),
    sqlStr(rec.dressCode),          // dress_code
    ...
    SCHEMA_VERSION,
    sqlStr(sourceType),             // source_type
    sqlStr(sourceTitle),            // source_title
    sqlStr(sourceUrl),              // source_url
...
-- Records exported: ${deduped.length}
...
for (let i = 0; i < deduped.length; i += BATCH_SIZE) {
  ...
  const rows = batch.map((rec, j) => buildRow(rec, i + j + 1))
  const isLastBatch = i + BATCH_SIZE >= deduped.length
  outputLines.push(rows.join(',\n') + (isLastBatch ? ';' : ','))
...
console.log(`Done. ${deduped.length} temples exported.`)
