...
const stateSet = [...new Set(deduped.map((r) => r.state).filter(Boolean))]
...
const buildRow = (rec) => {
  ...
  const values = [
    ...
    sqlStr(rec.image),
    sqlStr(rec.credit),
    sqlStr(rec.creditUrl),
    sqlStr(rec.bestTime),
    sqlStr(rec.timings),
    sqlStr(rec.dressCode),
...
for (let i = 0; i < deduped.length; i += BATCH_SIZE) {
  ...
  const rows = batch.map((rec) => buildRow(rec))
...
console.log(`Exported rows: ${deduped.length}`)
